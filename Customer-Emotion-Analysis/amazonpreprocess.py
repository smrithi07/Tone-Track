...
def process_topic(raw_topic, processed_topic, batch_size=100, bootstrap_servers='localhost:9092'):
    ...
    try:
        ...
        if batch_messages:
            texts = [clean_text(msg.get("review", "")) for msg in batch_messages]
            processed_texts = [
                " ".join([token.lemma_ for token in doc if token.is_alpha and not token.is_stop])
...
if __name__ == "__main__":
    process_topic(
        raw_topic="amazon_reviews",             # Your input Kafka topic
        processed_topic="amazon_reviews_clean", # Output Kafka topic
        batch_size=100
    )
